package xyz.lazyghosty.phant0m.manager.receiver
...
class BootCompleteReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (!isBootAction || !Phant0mSettings.getStartOnBoot()) {
            return
        }

        if (Phant0m.pingBinder()) {
            Log.d(TAG, "status: ${Phant0m.pingBinder()}")
            return
        }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R &&
            context.checkSelfPermission(WRITE_SECURE_SETTINGS) == PackageManager.PERMISSION_GRANTED &&
            Phant0mSettings.getLastLaunchMode() == Phant0mSettings.LaunchMethod.ADB
        ) {
            val pending = goAsync()
            startAdb(context) {
                safeFinish(pending)
            }
        } else if (Phant0mSettings.getLastLaunchMode() == Phant0mSettings.LaunchMethod.ROOT) {
            val pending = goAsync()
            startRoot(pending)
        } else {
