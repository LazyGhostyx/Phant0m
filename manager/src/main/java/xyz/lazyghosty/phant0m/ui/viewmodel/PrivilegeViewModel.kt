...
class PrivilegeViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun granted(uid: Int): Boolean {
        return (Phant0m.getFlagsForUid(uid, MASK_PERMISSION) and FLAG_ALLOWED) == FLAG_ALLOWED
    }

    fun grant(uid: Int) {
        Phant0m.updateFlagsForUid(uid, MASK_PERMISSION, FLAG_ALLOWED)
    ...
    fun revoke(uid: Int) {
        Phant0m.updateFlagsForUid(uid, MASK_PERMISSION, 0)
        privileges = privileges.toMutableMap().apply {
            this[uid]?.let {
