package xyz.lazyghosty.phant0m.manager.ui.webui.interfaces
...
import frb.phant0m.server.util.flattenOneLevel
    ...
    fun exec(cmd: String): String {
        return runCatching {
            runBlocking(Dispatchers.IO) {
                val result = Phant0mPluginService.execWithIO(
                    cmd = cmd,
                    useBusybox = false,
                    hideStderr = false
                )
...
//            }
            ...
            val future = Phant0mPluginService.execWithIOFuture(
                cmd = finalCommand,
    ...
    fun moduleInfo(): String {
        val pluginInfos = Phant0m.getPlugins()
    ...
    fun listPackages(type: String): String {
        val packageNames = Phant0m.getPackages(0)
            .filter { packageInfo ->
...
//        val pm = context.packageManager
        val packages = Phant0m.getPackages(0).mapNotNull { pkg ->
...
//        val pm = context.packageManager
        val packages = Phant0m.getPackages(0)
            .mapNotNull { pkg ->
                val appInfo = pkg.applicationInfo
                if (appInfo != null && (appInfo.flags and ApplicationInfo.FLAG_SYSTEM) == 0) {
                    pkg.packageName
...
//        val pm = context.packageManager
    ...
    fun getPackagesInfo(packageNamesJson: String): String {
        ...
        val jsonArray = JSONArray()
        val appMap = Phant0m.getPackages(0).associateBy { it.packageName }
