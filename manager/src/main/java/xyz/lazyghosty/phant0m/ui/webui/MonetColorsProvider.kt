...
import xyz.lazyghosty.phant0m.ui.theme.getVortexDarkColorScheme
import xyz.lazyghosty.phant0m.ui.theme.getVortexLightColorScheme
...
object MonetColorsProvider {
    ...
    fun getColorsCss(context: Context): String {
        ...
        val darkTheme = when (Phant0mSettings.getAppThemeId()) {
            1 -> true
            2 -> false
            else -> isDark
        }

        val dynamicColor: Boolean = Phant0mSettings.getEnableDynamicColor()

        val primaryColor = hexToColor(Phant0mSettings.getCustomPrimaryColor() ?: basePrimaryDefault.toHexString())
