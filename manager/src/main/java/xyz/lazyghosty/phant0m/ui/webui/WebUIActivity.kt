package xyz.lazyghosty.phant0m.manager.ui.webui
...
import xyz.lazyghosty.phant0m.ui.webui.interfaces.AxWebInterface
import xyz.lazyghosty.phant0m.ui.webui.interfaces.KsuWebInterface
import frb.phant0m.server.PluginInfo
import frb.phant0m.server.util.AxWebLoader
...
class WebUIActivity : ComponentActivity() {
    ...
    private suspend fun setupWebView() {
        plugin = Phant0m.getPluginById(intent.getStringExtra("id") ?: finishAndRemoveTask().let {
        ...
        val enableWebDebugging = Phant0mSettings.getEnableWebDebugging()

        WebView.setWebContentsDebuggingEnabled(developerOptionsEnabled && enableWebDebugging)

        val pluginDir =
            File(PathHelper.getWorkingPath(Phant0m.getPhant0mInfo().isRoot(),Phant0mApiConstant.folder.PARENT_PLUGIN), plugin.dirId)
        ...
        val axWebLoader = AxWebLoader.Builder()
            ...
            .addDomain("phant0m.js").addHandler { _, _, request ->
                Log.d("WebUIActivity", "request: " + request.url.toString())
                return@addHandler WebResourceResponse(
                    "application/javascript",
                    null,
                    assets.open("js/phant0m.js")
                )
            }.done()
            .addScheme("ksu")
        ...
        webView.apply {
            ...
            addJavascriptInterface(AxWebInterface(this@WebUIActivity, pluginDir), "Phant0m")
