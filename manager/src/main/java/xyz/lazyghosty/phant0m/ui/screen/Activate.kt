...
import xyz.lazyghosty.phant0m.ui.util.ClipboardUtil
...
fun ActivateScreen(navigator: DestinationsNavigator, viewModelGlobal: ViewModelGlobal) {
    ...
    if (phant0mInfo.isRunning() && !phant0mInfo.isNeedUpdate()) {
            ...
            Button(
                onClick = {
                    activateViewModel.startAdbTcp(context) { ai ->
                        ...
                        Log.e("Phant0mStartAdb", ai.message, ai.cause)
                        activateViewModel.setTryToActivate(false)
                    }
                }
            ) {
                if (AdbEnvironment.getAdbTcpPort() != Phant0mSettings.getTcpPort()) {
                    Icon(
...
fun WirelessDebuggingCard(
    ...
    val stepByStepUrl =
        "https://fahrez182.github.io/Phant0m/guide/user-manual.html#start-with-wireless-debugging"
        ...
        Column(
            ...
            LaunchedEffect(activateViewModel.devSettings) {
                if (activateViewModel.devSettings) {
                    ...
                    Log.d("Phant0m", "launchDevSettings")
