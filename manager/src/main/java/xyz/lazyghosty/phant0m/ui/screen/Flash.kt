...
import frb.phant0m.api.Phant0mPluginService.flashPlugin
import frb.phant0m.api.core.Phant0mSettings
import frb.phant0m.api.core.Starter
import frb.phant0m.api.utils.AnsiFilter
import xyz.lazyghosty.phant0m.R
import xyz.lazyghosty.phant0m.ui.component.AxSnackBarHost
import xyz.lazyghosty.phant0m.ui.component.KeyEventBlocker
import xyz.lazyghosty.phant0m.ui.component.rememberLoadingDialog
import xyz.lazyghosty.phant0m.ui.component.resolveDisplayName
import xyz.lazyghosty.phant0m.ui.theme.GREEN
import xyz.lazyghosty.phant0m.ui.theme.ORANGE
import xyz.lazyghosty.phant0m.ui.theme.RED
import xyz.lazyghosty.phant0m.ui.util.LocalSnackbarHost
...
fun FlashScreen(
    ...
    val developerOptionsEnabled = Phant0mSettings.getEnableDeveloperOptions()
            ...
            TopBar(
                ...
                onSave = {
                    scope.launch {
                        ...
                        val baseDir = PathHelper.getPath(Phant0mApiConstant.folder.PARENT_LOG)
                        if (!baseDir.exists()) {
                            baseDir.mkdirs()
                        }

                        val file = File(baseDir, "Phant0m_install_log_${date}.log")

                        try {
                            val fos = Phant0m.newFileService()
...
): Phant0mPluginService.FlashResult {
    for (installer in installers) {
        flashPlugin(installer, onStdout, onStderr).apply {
            if (code != 0) {
                return Phant0mPluginService.FlashResult(code, err, showReboot)
            }
        }
    }
    return Phant0mPluginService.FlashResult(0, "", true)
}
...
suspend fun flashIt(
    ...
    return when (flashIt) {
        ...
        is FlashIt.FlashUninstall -> Phant0mPluginService.resetManagerNative(onStdout, onStderr)
    }
}
