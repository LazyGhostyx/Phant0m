...
import xyz.lazyghosty.phant0m.Phant0mApplication.Companion.phant0mApp
...
import xyz.lazyghosty.phant0m.ui.util.DownloadListener
import xyz.lazyghosty.phant0m.ui.util.download
...
fun PluginList(
    ...
    suspend fun onModuleUpdate(
        ...
        val changelogResult = loadingDialog.withLoading {
            withContext(Dispatchers.IO) {
                runCatching {
                    phant0mApp.okhttpClient.newCall(
    ...
    suspend fun onModuleUninstall(plugin: PluginInfo) {
        ...
        val success = loadingDialog.withLoading {
            withContext(Dispatchers.IO) {
                Phant0mPluginService.uninstallPlugin(plugin.dirId)
            }
        }
    ...
    suspend fun onModuleRestore(plugin: PluginInfo) {
        ...
        val success = loadingDialog.withLoading {
            withContext(Dispatchers.IO) {
                Phant0mPluginService.restorePlugin(plugin.dirId)
            }
        }
                        ...
                        PluginItem(
                            ...
                            onCheckChanged = {
                                scope.launch {
                                    val success = loadingDialog.withLoading {
                                        withContext(Dispatchers.IO) {
                                            Phant0mPluginService.togglePlugin(
                                                plugin.dirId,
                                                !plugin.enabled
                                            )
                                        }
