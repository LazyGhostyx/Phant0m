package xyz.lazyghosty.phant0m.manager.adb
...
import frb.phant0m.adb.AdbClient
import frb.phant0m.adb.AdbKey
import frb.phant0m.adb.AdbMdns
import frb.phant0m.adb.PreferenceAdbKeyStore
import frb.phant0m.adb.util.AdbEnvironment
import frb.phant0m.adb.util.AdbWifiGate
import frb.phant0m.adb.util.WifiReadyGate
...
object AdbStarter {
    ...
    suspend fun startAdbClient(context: Context, port: Int, result: (AdbStateInfo) -> Unit = {}) {
        ...
        var activePort = port
        val tcpMode = Phant0mSettings.getTcpMode()
        val tcpPort = Phant0mSettings.getTcpPort()
        ...
        runCatching {
            ...
            Phant0mSettings.setLastLaunchMode(Phant0mSettings.LaunchMethod.ADB)
    ...
    suspend fun stopTcp(context: Context, port: Int, result: (AdbStateInfo) -> Unit = {}) {
        runCatching {
            ...
            val key = AdbKey(keyStore, "phant0m")
